...
function buildPlanPayload() {
  return {
    name: "Uso livre",
    price_cents: 0,
    price_label: "Uso livre",
    currency: PLAN_CURRENCY,
    interval: "sem limite",
    trial_days: 0,
    provider: "google_backup",
    monetization_enabled: false,
    google_backup_available: Boolean(GOOGLE_CLIENT_ID),
    requires_google_play_billing_on_play_store: false,
  };
}
...
function buildAccessMode({ user }) {
  if (user) {
    return {
      mode: "free_signed_in",
      can_generate: true,
      reason: "Uso liberado. A conta permanece disponivel apenas para recursos opcionais.",
    };
  }

  return {
    mode: "free_open_access",
    can_generate: true,
    reason: "Uso liberado sem bloqueio. Conecte o Google apenas se quiser backups.",
  };
}
...
function buildSnapshot({ user, subscription, trialPayload, access }) {
  return {
    ...
    access: {
      ...access,
      login_required_after_trial: false,
    },
    user: safeUser(user),
...
function resolveAccessState(db, req, { createTrial = false } = {}) {
...
async function handleCheckout(req, res) {
  sendJson(res, 410, {
    error:
      "As assinaturas ficaram desativadas por enquanto. O app esta em uso livre e o checkout foi removido.",
  });
}
...
async function handleDemoActivation(req, res) {
  sendJson(res, 410, {
    error:
      "Assinaturas locais tambem foram desativadas. O app agora funciona sem bloqueio.",
  });
}
...
server.listen(PORT, HOST, () => {
  ...
  console.log("Modo de acesso: livre, sem bloqueio de trial ou assinatura.");
  console.log(
    GOOGLE_CLIENT_ID
      ? "Backup Google: configurado."
      : "Backup Google: configure GOOGLE_CLIENT_ID para habilitar a conexao."
  );
  console.log(`Checkout atual no backend: ${PROVIDER_MODE}`);
